...
function openSupportPayment(){
    ...
    const card =
        document.querySelector(".support-money-card");

    const panel =
        document.getElementById("supportPaymentPanel");
...
function closeSupportPayment(){
    ...
    const bankDetails =
        document.getElementById("supportBankDetails");
...
const supportBankBtn =
    document.getElementById("supportBankBtn");

const supportBankDetails =
    document.getElementById("supportBankDetails");


if(supportBankBtn){

    supportBankBtn.addEventListener(
        "click",
        function(){

            if(!supportBankDetails)
                return;

            if(
                supportBankDetails.style.display ===
                "none"
            ){
...
/* Copy StudyMate bank account */

function copySupportAccount(){
    ...
    if(
        ...
        navigator.clipboard
            ...
            .catch(function(){
                ...
                alert(
                    "Could not copy the account number."
                );
...
/* =========================
   SUPPORT FLUTTERWAVE PAYMENT
========================= */

const supportFlutterwaveBtn =
    document.getElementById("supportFlutterwaveBtn");

if(supportFlutterwaveBtn){

    supportFlutterwaveBtn.addEventListener(
        "click",
        async function(){
            ...
            if(!user || !user.email){
                ...
                alert(
                    "Please log in to your StudyMate account before making a support payment."
                );
            ...
            const originalContent =
                supportFlutterwaveBtn.innerHTML;

            supportFlutterwaveBtn.disabled = true;

            supportFlutterwaveBtn.innerHTML = `
                <span class="support-method-icon">⏳</span>

                <span>
                    <strong>Starting payment...</strong>
            ...
            try{
                ...
                const response =
                    await fetch(
                        "https://studymate-ai-backend-op3o.onrender.com/create-support-payment",
                        {
                            method: "POST",
                ...
                if(
                    ...
                    throw new Error(
                        data.error ||
                        "Unable to start support payment."
                    );
                }
                ...
                console.error(
                    "StudyMate Support payment error:",
                    error
                );

                alert(
                    error.message ||
                    "Could not start the support payment. Please try again."
                );

                supportFlutterwaveBtn.innerHTML =
                    originalContent;

                supportFlutterwaveBtn.disabled =
                    false;
            }
...
function checkSupportPaymentResult(){
    ...
    if(supportStatus === "success"){
        ...
        alert(
            formattedAmount
                ? `❤️ Thank you for supporting StudyMate with ₦${formattedAmount}!`
                : "❤️ Thank you for supporting StudyMate!"
        );
    ...
    if(supportStatus === "failed"){
        ...
        alert(
            "❌ The StudyMate support payment was not completed. Please try again."
        );
...
function contactStudyMateSupport(){

    window.location.href =
        "mailto:supportstudymate@gmail.com?subject=StudyMate%20Support";
}

function openStudyMateFAQ(){

    const options =
        document.querySelector(".support-options");
...
function openStudyMateProblemReport(){
...
function closeStudyMateProblemReport(){
...
function sendStudyMateProblemReport(){

    const message =
        document
            .getElementById(
                "supportProblemMessage"
            )
            ?.value
            .trim();
    ...
    const subject =
        encodeURIComponent(
            "StudyMate Problem Report"
        );

    const body =
        encodeURIComponent(
            "StudyMate Account Email: " +
            accountEmail +
            "\n\nProblem:\n" +
            message
        );
    ...
    window.location.href =
        `mailto:supportstudymate@gmail.com?subject=${subject}&body=${body}`;
...
function openStudyMateIdeaPanel(){
...
function closeStudyMateIdeaPanel(){
...
function sendStudyMateIdea(){

    const message =
        document
            .getElementById(
                "studyMateIdeaMessage"
            )
            ?.value
            .trim();
    ...
    if(!message){
        ...
        alert(
            "Please enter your idea before sending."
        );
...
function goToStudyCenter(type){

    const studyCenter =
        document.getElementById("studyCenter");
