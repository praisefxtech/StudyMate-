...
/* =========================================================
   CURRENT FIREBASE AI ACCOUNT
========================================================= */

let activeAIUserId = null;
...
function resetAIVoiceButton(){
    ...
    if(aiMicButton){
        aiMicButton.textContent = "🎙️";
        aiMicButton.classList.remove("listening");
        aiMicButton.title = "Speak";
    }
}
...
function startAIListening(){
    ...
    if(!Recognition){
        ...
        alert(
            "Voice input is not supported on this device/browser. Try Chrome on Android."
        );
    ...
    aiSpeechRecognition.lang =
        "en-US";

    aiSpeechRecognition.continuous =
        false;

    aiSpeechRecognition.interimResults =
        true;

    aiSpeechRecognition.maxAlternatives =
        1;
...
if(aiInput){
    ...
    aiInput.classList.add(
        "ai-voice-text"
    );

    clearTimeout(
        aiInput._voiceTextTimer
    );
...
async function checkPremiumPayment(user = null) {
    ...
    if (!user) {
        ...
        alert(
            "Payment was successful, but your account could not be identified. Please log in again."
        );
    ...
    console.log(
        "💳 Payment return detected for:",
        user.uid
    );

    /*
       The backend has already verified the payment
       and saved Premium to Firestore.

       So we ONLY READ the Premium status here.
       We do NOT save it again from the frontend.
    */

    const isPremium =
        await loadStudyMatePremium(user);

    if (!isPremium) {

        console.error(
            "❌ Premium was not found in Firestore."
        );

        alert(
            "Your payment was successful, but Premium has not synced to your account yet. Please refresh the page."
        );
    ...
    /*
       Remove the payment parameters from the URL
...
async function saveStudyMatePremium(plan, user = null) {
...
/* =========================================================
   LOAD STUDYMATE PREMIUM
========================================================= */

async function loadStudyMatePremium(user = null) {
...
/* =========================================================
   ACCOUNT-SPECIFIC AI STORAGE
========================================================= */

function getAIStoragePrefix(){

    if(activeAIUserId){

        return `studyMate_${activeAIUserId}_`;

    }

    return "studyMate_guest_";

}


function getAIStorageKey(key){
...
function switchStudyMateAIAccount(user){

    const newUserId =
        user?.uid || null;


    /* Already using this account */

    if(activeAIUserId === newUserId){
    ...
    console.log(
        "🔄 Switching StudyMate AI account:",
        newUserId || "GUEST"
    );
    ...
    /*
     * Load this account's own AI data.
     */
    ...
    /*
     * Restore this account's current chat.
     */
...
function toggleAISpeech(text, button){
    ...
    if(
        ...
        if(aiSpeechButton){
            ...
            aiSpeechButton.textContent =
                "🔊";

            aiSpeechButton.classList.remove(
                "speaking"
            );
    ...
    if(
        ...
        alert(
            "Voice reading is not supported on this device/browser."
        );
...
if(role === "assistant"){
    ...
    Array.from(responseLines).forEach(
        function(line, index){
    ...
    /*
     * Add voice button only to
     * StudyMate AI messages.
     */
...
/* =========================================================
   FIREBASE AUTH → PREMIUM CHECK
========================================================= */

function startStudyMatePremiumCheck() {

    if (!window.firebaseAuth || !window.firebaseUser) {

        console.log(
            "⏳ Firebase Auth not ready yet. Retrying Premium check..."
        );

        setTimeout(
            startStudyMatePremiumCheck,
            500
        );
    ...
    console.log(
        "🔥 Firebase Auth ready. Starting Premium listener..."
    );
...
switchStudyMateAIAccount(user);
            ...
            await checkPremiumPayment(user);
